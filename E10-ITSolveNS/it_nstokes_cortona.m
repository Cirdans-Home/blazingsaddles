...
% get geometry information
   if geo_domain==1,
      load ./datafiles/square_stokes_nobc.mat,
   elseif geo_domain==3,
      load ./datafiles/step_stokes_nobc.mat
   else
...
%    compute new MG data or reload existing data?
      ...
      if compute_mg==2
         cd datafiles, load amgdata_cd.mat
      else
         amg_gridF = amg_grids_setup(Anst);
         fprintf('\n')
         cd datafiles, save amgdata_cd.mat amg_gridF
      end
...
etoc = toc;
fprintf("\tElapsed time in the build phase is %1.2e\n",etoc);
%   
%
...
fprintf("\tElapsed time in the solve phase is %1.2e\n",etoc);
