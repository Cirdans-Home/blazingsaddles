...
q_in = 3; % Q1-Q1/Q1-P0/Q2-Q1/Q2-P1: 1/2/3/4
% ----------------------------------------------------------------------- %
qmethod=q_in-1;
if qmethod==2,
...
linstrategy = 1; %0 Direct/1 Augmented Lagrangian
nlmethod = 1; % Picard/Newton/hybrid linearization 1/2/3
nlmethod=nlmethod-1;
if nlmethod==0,
    ...
    if     qmethod>1,  
        switch linstrategy
            ...
            case 1
                ...
                addpath('../');
                gamma = 100; 
                % Using identity instead of the pressure mass matrix
                % W = speye(size(Bst,1),size(Bst,1));
                % Using the pressure mass matrix
                W = Q;
                Aaug = [Anst + gamma*Bst'*(W\Bst),Bst';
                        Bst,sparse(np,np)];
                nlresaug = nlres;    
                nlresaug(1:size(Anst,1)) = nlres(1:size(Anst,1)) + gamma*Bst'*(W\nlres(size(Anst,1)+1:end));
                
                dxns = gmres(Aaug,nlresaug,[],1e-9,200,@(x) augmented_lagrangian(Anst,Bst,W,gamma,x) );
                dxns = -dxns;
                if it_p == 1
                    % If it is the first iteration we perform a brute force
                    % spectral analysis
                    PAug = augmented_lagrangian(Anst,Bst,W,gamma,Aaug);
                    w = eig(full([Anst,Bst';Bst,sparse(np,np)]));
                    v = eig(full(PAug));
                    figure(1000)
                    plot(real(w),imag(w),'o',real(v),imag(v),'x','MarkerSize',12)
                    legend({'Original EVs','Preconditioned Spectrum'},'FontSize',16);
                end
            otherwise
                error("I don't know what are you talking about");
        end
